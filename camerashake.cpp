...
extern "C" {
    ...
    /* Plugin instance data structure holding parameters and frame dimensions */
    ...
    /* Plugin metadata for host applications like Kdenlive */
    ...
    /* Parameter metadata for UI sliders and animation support */
    ...
    /* Allocate and initialize new plugin instance with default values */
    ...
    /* Update a specific parameter value from host */
    ...
    /* Generate organic irregular noise using layered sine waves with irrational frequency ratios */
    ...
    /* Main processing function: applies camera shake transform to each frame */
    void f0r_update(f0r_instance_t instance, double time, const uint32_t* inframe, uint32_t* outframe) {
        ...
        /* Zoom scaling with safety clamp */
        ...
        /* Precomputed affine transform coefficients for efficient source coordinate calculation */
        const double inv_scale = 1.0 / scale;
        const double a = cos_t * inv_scale;   /* x scaling + rotation component */
        const double b = -sin_t * inv_scale;  /* y to x rotation component */
        const double c = sin_t * inv_scale;   /* x to y rotation component */
        const double d = cos_t * inv_scale;   /* y scaling + rotation component */
        const double tx = cx + shake_x - (cx * a + cy * b);
        const double ty = cy + shake_y - (cx * c + cy * d);

        /* Outer loop over destination rows - precompute y-dependent base */
        for (int y = 0; y < h; ++y) {
            ...
            for (int x = 0; x < w; ++x) {
                /* Compute source coordinates using affine transform */
                ...
                if (blur_radius <= 0) {
                    /* Direct bilinear sampling for sharp result */
                    bilinear_sample(inframe, w, h, src_x, src_y, out_ptr);
                } else {
                    /* Integer center for blur sampling */
                    ...
                    /* Simple cross blur: horizontal samples including center */
                    ...
                    /* Vertical samples excluding center to avoid double-counting */
                    for (int i = -blur_radius; i <= blur_radius; i += 2) {
                        if (i == 0) continue;
                        int sy = iy + i;
                        if (sy >= 0 && sy < h) {
