...
extern "C" {
    ...
    struct CameraShakeInstance {
        double params[9];           // 0: amplitude_x, 1: amplitude_y, 2: rotation,
    ...
    double compute_speed_slowdown_phase_offset(CameraShakeInstance* inst, double time, double speed_factor) {
        ...
        if (inst->params[5] > 0.001) {
            ...
            double normalized = (slow_noise + 1.20) / 2.40 * 1.20 - 0.175; // around 17% peak (towards negative, manifests as strength)
    ...
    void f0r_update(f0r_instance_t instance, double time, const uint32_t* inframe, uint32_t* outframe) {
        ...
        double base_t = time * speed_factor + inst->params[8];
