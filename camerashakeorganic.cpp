...
extern "C" {
    ...
    struct CameraShakeInstance {
        double amp_x, amp_y, rotation, zoom, speed, speed_var, blur, seed;
    ...
    void f0r_get_plugin_info(f0r_plugin_info_t* info) {
        ...
        info->num_params = 8;
        info->explanation = "Organic camera shake with irregular noise, fine micro-adjust, bilinear sampling, and speed variation.";
    }

    /* Parameter metadata for UI sliders and animation support - REORDERED */
    void f0r_get_param_info(f0r_param_info_t* info, int param_index) {
        ...
        else if (param_index == 5) { info->name = "speed_variation"; info->type = F0R_PARAM_DOUBLE; }
    ...
    f0r_instance_t f0r_construct(unsigned int width, unsigned int height) {
        ...
        inst->zoom = 100.0; inst->speed = 50.0; inst->speed_var = 30.0;
        inst->blur = 0.0; inst->seed = 0.0;
    ...
    /* Update a specific parameter value from host - REORDERED */
    void f0r_set_param_value(f0r_instance_t instance, f0r_param_t param, int param_index) {
        ...
        else if (param_index == 5) inst->speed_var = val;
        else if (param_index == 6) inst->zoom = val;
        else if (param_index == 7) inst->seed = val;
    }

    /* Retrieve current value of a parameter - REORDERED */
    void f0r_get_param_value(f0r_instance_t instance, f0r_param_t param, int param_index) {
        ...
        else if (param_index == 5) *((double*)param) = inst->speed_var;
    ...
    void f0r_update(f0r_instance_t instance, double time, const uint32_t* inframe, uint32_t* outframe) {
        ...
        /* === SPEED VARIATION - ONLY SLOWS DOWN === */
        const double var_noise = organic_noise(t * 0.2);
        double speed_mod = 1.0 - ((var_noise + 1.0) * 0.5) * (inst->speed_var / 100.0);
        if (speed_mod < 0.1) speed_mod = 0.1;
        const double speed_factor = (inst->speed / 25.0) * speed_mod;
